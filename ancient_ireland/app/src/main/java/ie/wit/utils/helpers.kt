...
fun createLoader(activity: FragmentActivity) : AlertDialog {
    ...
    loader.setIcon(R.mipmap.ic_launcher_shamrock_round)
...
fun uploadImageView(app: AiApp, imageView: ImageView) {
    ...
    uploadTask.addOnFailureListener { object : OnFailureListener {
        override fun onFailure(error: Exception) {
            Log.v("Rate", "uploadTask.exception" + error)
        }
    }
...
fun updateAllRatings(app: AiApp) {
    val userId = app.currentUser!!.uid
    val userEmail = app.currentUser!!.email
    val ratingRef = app.database.ref.child("ratings")
                                  .orderByChild("email")
    val userratingRef = app.database.ref.child("user-ratings")
                                  .child(userId).orderByChild("uid")

    ratingRef.equalTo(userEmail).addListenerForSingleValueEvent(
    ...
    userratingRef.addListenerForSingleValueEvent(
        object : ValueEventListener {
        override fun onCancelled(error: DatabaseError) {}
        override fun onDataChange(snapshot: DataSnapshot) {
            snapshot.children.forEach {
...
fun writeImageRef(app: AiApp, imageRef: String) {
    val userId = app.currentUser!!.uid
...
fun validatePhoto(app: AiApp, activity: Activity) {
    ...
    if (googlePhotoExists || imageExists) {
        ...
        activity.navView.getHeaderView(0).imageView
            .setImageResource(R.mipmap.ic_launcher_shamrock_round)
        uploadImageView(app, activity.navView.getHeaderView(0).imageView)
    }
}

fun checkExistingPhoto(app: AiApp, activity: Activity) {
